...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form method="post" action="Select?currentPageNo=0">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryStatus" class="form-control">
									   ...
									   <c:forEach var="list" items="${dataDictionaries}">
									   		
									   		<c:if test="${list.typeName =='APP状态' }">	
									   		<option <c:if test="${list.valueId == queryStatus }">selected="selected"</c:if>
									   		value="${list.valueId}">${list.valueName}</option>
									   		</c:if>
									   </c:forEach>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryFlatformId" class="form-control">
									  ...
									  <c:if test="${list.typeName =='所属平台' }">	
									   		<option <c:if test="${list.valueId == queryFlatformId }">selected="selected"</c:if>
									   		value="${list.valueId}">${list.valueName}</option>
									  </c:if>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select id="queryCategoryLevel1" name="queryCategoryLevel1" class="form-control">
									   ...
									   <option value="">--请选择-- </option>
									   <c:forEach var="list" items="${appCategories}">
											
										<option <c:if test="${list.id == queryCategoryLevel1 }">selected="selected"</c:if>
									   			value="${list.id}">${list.categoryName}</option>
	...
	<div class="x_panel">
		<div class="x_content">
			...
			<div id="datatable-responsive_wrapper"
				...
				<div class="row">
					<div class="col-sm-12">
					<a href="${pageContext.request.contextPath}/user/appinfoadd" class="btn btn-success btn-sm">新增APP基础信息</a>
                       ...
                       <li>
                        				<c:choose>
											<c:when test="${appInfo.status == 2 || appInfo.status == 5}">
												<a class="saleSwichOpen" saleSwitch="open" appinfoid=${appInfo.id }   appsoftwarename=${appInfo.statusName } data-toggle="tooltip" data-placement="top" title="" data-original-title="恭喜您，您的审核已经通过，您可以点击上架发布您的APP">上架</a>
											</c:when>
							...
							<ul class="pagination">
								<c:if test="${currentPageNo > 1}">
									<li class="paginate_button previous"><a
										href="?currentPageNo=1">首页</a>
									</li>
									<li class="paginate_button "><a
										href="?currentPageNo=${currentPageNo-1}">上一页</a>
									</li>
								</c:if>
								<c:if test="${currentPageNo < coun }">
									<li class="paginate_button "><a
										href="?currentPageNo=${currentPageNo+1}">下一页</a>
									</li>
									<li class="paginate_button next"><a
										href="?currentPageNo=${coun}"
